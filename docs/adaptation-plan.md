...
- **`frontend/src/pages/rules-reference/rules-reference.tsx`** — Removed the two-tab layout (Grimoire tab was AH-specific). Rules page now shows the glossary directly.
- **`frontend/src/locales/en.json`** — Updated `rules.title` to "Rules glossary" and `rules.search_placeholder` to "Search glossary…".
